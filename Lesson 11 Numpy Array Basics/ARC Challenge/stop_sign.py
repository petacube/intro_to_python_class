...
color_map ={"red":("yellow","corners"),
            "blue":("orange","sides")
            }

json_files = list(filter(lambda x: file_to_concept_map['stop_sign'] in x,json_files))
...
for json_file in json_files:
    ...
    for train_sample in data['train']:
        ...
        if case_number >0:
            ...
            visualize_array(input_arr,custom_colormap=stop_cmap,custom_norm=stop_norm)

            computed_output = np.zeros(input_arr.shape)
            for i in range(num_rows_input):
                for j in range(num_columns_input):
                    if stop_signs_reverse.get(input_arr[i,j]) is not None:
                        if stop_signs_reverse[input_arr[i,j]] in color_map.keys():
                            surround_color, surround_style = color_map[stop_signs_reverse[input_arr[i,j]]]
                            if surround_style == "corners":
                                computed_output[i,j] = input_arr[i,j]
                                computed_output[i-1,j-1] = stop_signs_colors[surround_color]
                                computed_output[i-1,j+1] = stop_signs_colors[surround_color]
                                computed_output[i+1,j+1] = stop_signs_colors[surround_color]
                                computed_output[i+1,j-1] = stop_signs_colors[surround_color]
                            elif (surround_style == "sides"):
                                computed_output[i,j] = input_arr[i,j]
                                computed_output[i,j-1] = stop_signs_colors[surround_color]
                                computed_output[i,j+1] = stop_signs_colors[surround_color]
                                computed_output[i+1,j] = stop_signs_colors[surround_color]
                                computed_output[i-1,j] = stop_signs_colors[surround_color]
                            else:
                                raise Exception(f"Unknown style!! {surround_style}")
                        else:
                            if input_arr[i,j] != 0:
                                computed_output[i,j] = input_arr[i,j]
                    else:
            ...
            if  np.array_equal(computed_output, output_arr):
                ...
                print("wrong answer")
                visualize_array(input_arr,custom_colormap=stop_cmap,custom_norm=stop_norm)
                visualize_array(output,custom_colormap=stop_cmap,custom_norm=stop_norm)
                visualize_array(computed_output,custom_colormap=stop_cmap,custom_norm=stop_norm)
                pass
